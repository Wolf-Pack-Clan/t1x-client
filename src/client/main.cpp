...
{
    std::string crashFilename = "t1x_crash.log";
...
{
    return clientNamedMohaa ? "mohaa.exe" : "CoDUOMP.exe";
}
...
static DWORD WINAPI stub_GetModuleFileNameA(HMODULE hModule, LPSTR lpFilename, DWORD nSize)
{
    ...
    if (!strcmp(PathFindFileNameA(lpFilename), "t1x.exe"))
    {
...
static DWORD WINAPI stub_GetModuleFileNameW(HMODULE hModule, LPWSTR lpFilename, DWORD nSize)
{
    ...
    if (!strcmp(PathFindFileNameA(pathStr.c_str()), "t1x.exe"))
    {
...
static FARPROC load_binary()
{
    ...
    std::string data_coduomp;

    if (!read_file(client_filename, &data_coduomp))
    {
        std::stringstream ss;
    ...
    return loader.load(self, data_coduomp);
}
